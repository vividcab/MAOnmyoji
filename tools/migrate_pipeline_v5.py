...
def remove_jsonc_docs_for_parsing(text: str) -> str:
    """
...
def parse_jsonc(text: str) -> Any:
    ...
    clean_text = remove_jsonc_docs_for_parsing(text)
                ...
                if orig_next != new_next:
                    if "next" in original_node_data:
                        ...
                        if next_line_match:
                            ...
                            inline_doc = next_line_match.group(3) or ""
                            ...
                            if "\n" in new_next_str:
                                # 多行格式
                                new_field = f'{field_indent}"next": {new_next_str}{trailing_comma}{inline_doc}'
                            else:
                                # 单行格式
                                new_field = f'{field_indent}"next": {new_next_str}{trailing_comma}{inline_doc}'
            ...
            if "on_error" in migrated_node_data:
                ...
                if orig_on_error != new_on_error:
                    ...
                    if on_error_match:
                        ...
                        inline_doc = on_error_match.group(3) or ""
                        ...
                        # 构建新的字段文本
                        if "\n" in new_on_error_str:
                            new_field = f'{field_indent}"on_error": {new_on_error_str}{trailing_comma}{inline_doc}'
                        else:
                            new_field = f'{field_indent}"on_error": {new_on_error_str}{trailing_comma}{inline_doc}'
...
def migrate_pipeline_file(
    ...
    if not dry_run:
        ...
        with open(file_path, "w", encoding="utf-8", newline="\n") as f:
            result_text = rebuild_json_with_docs(
